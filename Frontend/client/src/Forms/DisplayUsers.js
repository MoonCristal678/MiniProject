...
const DisplayUsers = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('miniproject10-backend.onrender.com/v1/api/users', {
        credentials: 'include',
      });
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      const response = await fetch('https://miniproject10-backend.onrender.com/v1/deleteUser', {
        method: 'POST',
        credentials: 'include',
        headers: {
